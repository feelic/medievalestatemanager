function Localisation (locale) {
	...
	if (!this.localisations[this.locale]) {
		throw new Error('Unknown locale ' + this.locale + ', did you forget to load it ?');
	}
	...
	*/
	...
	this.getRandomSaintName = function () {
		...
		if (!this.existingSaints) {
			this.existingSaints = {'f': [], 'm': []};
		}

		var g = '';
		if (randomBoolFromRate(0.5)) {
			g = 'm';
		}
		else {
			g = 'f';
	...
	this.getGen = function (name) {
		...
		if (isVowel(name[0])) {
			g = this.localisations[this.locale].genV;
		}
		return g;
